...
## per predir la y.
vars_x_opt = [] #vars_x.copy()

# Preparem una llista de candidats
#candidates = []
last_pvalor_max = -1
...
while(len(vars_x)==0 or not fi):
    ...
    for vi_indep in vars_x :
        vars_x_tmp = vars_x_opt.copy() # Variable  que ens servirà per guardar         
    ...
    if min_pvalor < SL:
        ...
        print("Traiem el valor {}",min_pvalor_key)
        print(vars_x[0])    
        vars_x.remove('R&D Spend')
    else:
        #Finalitzem perquè ja no hi ha cap model amb pvalor < SI
        fi = True
